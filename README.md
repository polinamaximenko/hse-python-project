...
- [Интерфейс](#Интерфейс)
- [Ограничения и перспективы](#Ограничения-и-перспективы-пилотного-проекта)
- [Визуализация работы приложения](#Визуализация-работы-приложения)
...
- **Модель:** `gpt-oss:120b-cloud` — это открытая большая языковая модель (Open‑Weight), доступная через Ollama Cloud. Она предназначена для мощного рассуждения, генерации текста и других задач, где требуется глубокое понимание семантики.
...
> [Подробнее об Ollama](https://ollama.com/)  
> [Справка по модели `gpt-oss:120b-cloud`](https://ollama.com/library/gpt-oss%3A120b-cloud)
...
## Ограничения и перспективы пилотного проекта
...
- Поскольку сейчас модель для создания эмбеддингов запускается на CPU, большие документы будут обрабатываться довольно долго. На настоящем этапе мы отдаем предпочтение качеству, комбинируя его с апробацией нового интересного подхода, а не скорости.
...
```
### 2. Структура проекта
Убедитесь, что директория приложения содержит папку `templates`, в которой находится файл `index.html`:
```text
your_project/
├── app.py
└── templates/
